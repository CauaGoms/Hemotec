...
class TestAdm_unidadeRepo:
    ...
    def test_obter_todos_vazia(self, test_db, lista_cidades_exemplo, lista_instituicoes_exemplo, lista_planos_exemplo, lista_assinaturas_exemplo, lista_licencas_exemplo, lista_adm_unidades_exemplo, lista_usuarios_exemplo, lista_unidades_coleta_exemplo):
        #Arrange
        cidade_repo.criar_tabela()
        for cidade in lista_cidades_exemplo:
            cidade_repo.inserir(cidade)
    ...
    def test_obter_por_id_existente(self, test_db, cidade_exemplo, instituicao_exemplo, plano_exemplo, assinatura_exemplo, licenca_exemplo, unidade_coleta_exemplo, adm_unidade_exemplo, usuario_exemplo):
        #Arrange
        cidade_repo.criar_tabela()
        id_cidade = cidade_repo.inserir(cidade_exemplo)
        ...
        #Act
        dados_db = adm_unidade_repo.obter_por_id(id_tabela_inserida)
        #Assert
        assert dados_db.cod_usuario == id_tabela_inserida, "O ID da unidade inserida não confere"
        assert dados_db.nome == "nome teste", "O nome inserido não confere"
        assert dados_db.email == "email teste", "O email inserido não confere"
        assert dados_db.senha == "senha teste", "O ID da unidade inserida não confere"
        assert dados_db.cpf == "cpf teste", "O ID da unidade inserida não confere"
        ...
        assert dados_db.cep_usuario == "cep_usuario teste", "O nome inserido não confere"
        assert dados_db.telefone == "telefone teste", "O nome inserido não confere"
        assert dados_db.cod_adm == adm_unidade_exemplo.cod_adm, "O nome inserido não confere"
        assert dados_db.cod_unidade == id_unidade, "O nome inserido não confere"
        assert dados_db.permissao_envio_campanha == True, "O nome inserido não confere"
        assert dados_db.permissao_envio_notificacao == True, "O nome inserido não confere"

    def test_obter_por_id_inexistente(self, test_db, cidade_exemplo, instituicao_exemplo, plano_exemplo, assinatura_exemplo, licenca_exemplo, unidade_coleta_exemplo, adm_unidade_exemplo, usuario_exemplo):
        #Arrange
        cidade_repo.criar_tabela()
        ...
        adm_unidade_repo.criar_tabela()
        #Act
        dados_db = adm_unidade_repo.obter_por_id(999)
        #Assert
