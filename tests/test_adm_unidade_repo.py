...
class TestAdm_UnidadeRepo:
    ...
    def test_inserir(self, test_db, adm_unidade_exemplo, unidade_coleta_exemplo, cidade_exemplo, licenca_exemplo, usuario_exemplo, assinatura_exemplo, plano_exemplo, instituicao_exemplo):
        #Arrange
        with get_connection() as conn:
            ...
            #Act
            conn.commit()
            id_tabela_inserida = adm_unidade_repo.inserir(adm_unidade_exemplo, cursor)
            #Assert
        dados_db = adm_unidade_repo.obter_por_id(adm_unidade_exemplo.cod_usuario)
        assert dados_db is not None, "O adm_unidade inserido não deveria ser None"
        assert dados_db.cod_usuario == 1, "O ID do adm_unidade inserido deveria ser igual a 1"
        assert dados_db.cod_adm == 1, "O ID do adm_unidade inserido deveria ser igual a 1"
        assert dados_db.cod_unidade == 1, "O ID da unidade inserido deveria ser igual a 1"
        assert dados_db.permissao_envio_campanha == True, "O email do usuário inserido não confere"
        assert dados_db.permissao_envio_notificacao == True, "A sigla do estado inserida não confere"
        assert dados_db.nome == "nome teste", "O CPF do usuário inserido não confere"
        assert dados_db.email == "email teste", "A data de nascimento do usuário inserido não confere"
        assert dados_db.senha == "senha teste", "O status do usuário inserido não confere"
        assert dados_db.cpf == "cpf teste", "A data de cadastro do usuário inserido não confere"
        assert dados_db.data_nascimento.strftime('%Y-%m-%d') == "2025-01-01", "A rua do usuário inserido não confere"
        assert dados_db.status == True, "O bairro do usuário inserido não confere"
        assert dados_db.data_cadastro.strftime('%Y-%m-%d') == "2025-01-01", "A cidade do usuário inserido não confere"
        assert dados_db.rua_usuario == "rua_usuario teste", "O CEP do usuário inserido não confere"
        assert dados_db.bairro_usuario == "bairro_usuario teste", "O telefone do usuário inserido não confere"
