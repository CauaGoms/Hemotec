...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function handleSubmit(event) {
        ...
        try {
            ...
            const response = await fetch('/colaborador/doacao/triagem', {
                method: 'POST',
                body: formDataToSend
            });
            ...
            submitBtn.innerHTML = '<i class="fas fa-check"></i> Finalizar Triagem';
            
            if (result.success) {
                // Criar objeto de resultado compatível com a função showResult
                triagemResult = {
                    status: result.apto ? 'apto' : 'inapto_temporario',
                    impedimentos: result.impedimentos || [],
                    recomendacoes: result.apto 
                        ? [
                            'Mantenha-se hidratado antes da doação',
                            'Faça uma refeição leve e nutritiva antes de vir ao hemocentro',
                            'NÃO é necessário estar em jejum para doar sangue',
                    ...
                    cod_doacao: result.cod_doacao,
                    cod_prontuario: result.cod_prontuario
                };
                
                // Verificar se há impedimentos permanentes
                const impedimentosPermanentes = ['Diabetes', 'Histórico de câncer', 'Hepatite', 'Uso de drogas ilícitas'];
                if (result.impedimentos && result.impedimentos.some(imp => impedimentosPermanentes.some(permImp => imp.includes(permImp)))) {
                    triagemResult.status = 'inapto_permanente';
                    triagemResult.recomendacoes = [
                        'Infelizmente você não pode doar sangue devido às condições identificadas',
                        'Consulte um médico para mais informações',
                        'Considere outras formas de ajudar, como divulgar campanhas de doação'
                    ];
                ...
                // Verificar se precisa avaliação médica
                const precisaAvaliacao = ['Hipertensão', 'Cardiopatia', 'Uso de medicamentos'];
                if (result.impedimentos && result.impedimentos.some(imp => precisaAvaliacao.some(av => imp.includes(av))) && triagemResult.status !== 'inapto_permanente') {
                    triagemResult.status = 'avaliacao_medica';
                    triagemResult.recomendacoes = [
                        'Será necessária uma avaliação médica no hemocentro',
                        'Traga seus exames médicos recentes',
                        'Liste todos os medicamentos que você usa',
                ...
                showAlert('Erro', result.message || 'Erro ao processar triagem. Tente novamente.', 'error');
            }
            ...
            showAlert('Erro', 'Erro ao processar triagem. Por favor, tente novamente.', 'error');
        }
    }
