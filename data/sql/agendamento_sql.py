CRIAR_TABELA = """
CREATE TABLE IF NOT EXISTS agendamento (
cod_agendamento INTEGER PRIMARY KEY AUTOINCREMENT,
cod_colaborador INTEGER,
cod_usuario INTEGER NOT NULL,
data_hora TEXT NOT NULL,
status INTEGER NOT NULL,
tipo_agendamento TEXT NOT NULL,
local_agendamento INTEGER NOT NULL,
FOREIGN KEY (cod_colaborador) REFERENCES colaborador(cod_colaborador),
FOREIGN KEY (cod_usuario) REFERENCES usuario(cod_usuario),
FOREIGN KEY (local_agendamento) REFERENCES unidade_coleta(cod_unidade)
)
"""

INSERIR = """
INSERT INTO agendamento (cod_colaborador, cod_usuario, data_hora, status, tipo_agendamento, local_agendamento) 
VALUES (?, ?, ?, ?, ?, ?)
"""

OBTER_TODOS = """
SELECT a.cod_agendamento, a.cod_colaborador, u.cod_usuario, a.data_hora, a.status, a.tipo_agendamento, a.local_agendamento
FROM agendamento a
INNER JOIN usuario u ON a.cod_usuario = u.cod_usuario
INNER JOIN unidade_coleta uc ON a.local_agendamento = uc.cod_unidade
LEFT JOIN colaborador c ON a.cod_colaborador = c.cod_colaborador
""" 

UPDATE = """
UPDATE agendamento
SET data_hora = ?, status = ?, tipo_agendamento = ?
WHERE cod_agendamento = ?;
"""

DELETE = """
DELETE FROM agendamento
WHERE cod_agendamento = ?;
"""

OBTER_POR_ID = """
SELECT a.cod_agendamento, a.cod_colaborador, u.cod_usuario, a.data_hora, a.status, a.tipo_agendamento, a.local_agendamento
FROM agendamento a
INNER JOIN usuario u ON a.cod_usuario = u.cod_usuario
INNER JOIN unidade_coleta uc ON a.local_agendamento = uc.cod_unidade
LEFT JOIN colaborador c ON a.cod_colaborador = c.cod_colaborador
WHERE a.cod_agendamento = ?;
"""

OBTER_PENDENTES_POR_USUARIO = """
SELECT a.cod_agendamento, a.cod_colaborador, a.cod_usuario, a.data_hora, a.status, a.tipo_agendamento, a.local_agendamento
FROM agendamento a
WHERE a.cod_usuario = ? AND a.status = ?;
"""